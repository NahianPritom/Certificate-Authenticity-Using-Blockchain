    ...
    async componentWillMount() {
      ...
      onSubmit = async (event) => {
        ...
        ipfs.add(this.state.buffer, (error, result) => {
           ...
           */
          this.state.contract.methods.CreateNotice(roll,Edate, result[0].hash).send({ from: this.state.account }).then((r) => {
            
          })
          testing();
          setTimeout(() => window.alert('Notice Added Successfully!!'), 15000)
        ...
        return (
            <div>
                <h1 className="mt-5 mr-auto ml-auto" >Admin</h1>
                    ...
                    <label>
                        <b>Notice No :</b>
                        <input className="ml-3" type="text" name="roll" placeholder="Enter Notice No" />
